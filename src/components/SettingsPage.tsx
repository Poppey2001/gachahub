...
export function SettingsPage() {
  ...
  useEffect(() => {
    backendPlatform().then(setPlatform);
  }, []);

  useEffect(() => {
    if (!requestedSettingsGameId) return;
    if (games.some((game) => game.id === requestedSettingsGameId)) {
      setTarget({ type: 'game', gameId: requestedSettingsGameId });
    }
    clearRequestedSettingsGame();
  }, [requestedSettingsGameId, games, clearRequestedSettingsGame]);

  const selectedGame = useMemo(() => {
    if (target.type !== 'game') return undefined;
    return games.find((game) => game.id === target.gameId);
  }, [games, target]);

  useEffect(() => {
    if (target.type === 'game' && !selectedGame) {
  ...
  return (
    <main className="page-shell settings-page-shell">
      <header className="page-header settings-page-header">
        <div><span className="page-eyebrow">CONFIGURATION</span><h1>Einstellungen</h1><p>Launcher-Defaults und installationsspezifische Overrides.</p></div>
      ...
      <div className="settings-layout">
        ...
        <div className="settings-workspace">
          {target.type === 'global' || !selectedGame ? (
            <GlobalSettingsPanel platform={platform} />
          ) : (
            <GameSettingsPanel game={selectedGame} />
          )}
        </div>
      </div>
    </main>
